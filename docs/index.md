---
layout: home

hero:
  name: SIFT Map Tracker
  text: 洛克王国：世界地图追踪工具
  tagline: 通过屏幕截图识别位置，提供路线导航、资源标记与地图工具；不读取游戏内存。
  image:
    src: /brand/logo.png
    alt: SIFT Map Tracker
  actions:
    - theme: brand
      text: 电脑端
      link: /getting-started/
    - theme: alt
      text: 手机端（精简版）
      link: /android/
    - theme: alt
      text: 下载路线
      link: https://www.mapforge.top/packs
    - theme: alt
      text: 自动手碟（独立工具）
      link: /handpan/

features:
  - title: 截图定位
    details: 使用 SIFT、模板匹配等图像算法定位玩家，不读取进程内存。
  - title: 路线导航
    details: 支持单路线、多路线切换、导航总览、HUD 与灵动岛提示。
  - title: 屏幕识别
    details: Windows 客户端内置的第二个工作台，可按需识别精灵蛋，并监测童话事件、突出显示血脉与累计触发次数。
    link: /screen-recognition/
    linkText: 查看电脑端屏幕识别
  - title: 地图工具
    details: 编辑路线和资源点，叠加图层进行地图标记并导出 PNG。
  - title: 手机端（精简版）
    details: 在 Android 手机上提供定位、路线与悬浮 HUD；编辑与导出等完整功能请使用电脑端。
---

## 当前版本

本 Wiki 当前对应电脑端 **v0.9.6** 与手机端 **v0.4.4**。正文始终以各平台最新稳定版为准，旧版本差异记录在[版本记录](/releases/)中。

## 手机端（精简版）

手机端不依赖电脑，通过系统屏幕采集完成定位，并以悬浮 HUD 显示地图和路线。手机端只保留导航所需的核心能力；移动端权限、录屏授权和小地图框选流程请从 [手机端指南](/android/) 开始。

## 自动手碟（独立工具）

导入 MIDI、试听和修改九音曲谱，并通过驱动向选定窗口发送按键。它不是地图追踪客户端内置功能，使用存在账号和系统驱动风险，请先阅读[自动手碟说明](/handpan/)，再按[五分钟快速开始](/handpan/quick-start)操作。

::: warning 使用限制
项目免费发布，仅供学习交流，禁止用于商业用途。请只从项目声明的官方渠道获取软件，不要为本项目付费。
:::

## 获取帮助

- 使用中断或报错：先查看[故障排查](/troubleshooting/)。
- 路线下载与分享：访问 [MapForge 路线分享站](https://www.mapforge.top/packs)。
- 软件发布与动态：[B站 sicuncat](https://space.bilibili.com/1570010855) / [抖音 偏差](https://www.douyin.com/user/MS4wLjABAAAAEVsQb2aABflGGf7fabxefdFMzV6GFwROH2jmYVMv0-ExGRsDxHaVjpgADCpsxoCs)。
